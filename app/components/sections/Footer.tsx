    ...
    <footer className="relative mt-10 overflow-hidden pb-10 pt-24">
      ...
      <div className="relative flex flex-col items-center gap-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm text-muted">
          ...
          <a href={profile.facebook} target="_blank" rel="noreferrer" className="transition-colors hover:text-cyan">Facebook</a>
          <a href={profile.youtube} target="_blank" rel="noreferrer" className="transition-colors hover:text-cyan">YouTube</a>
          <a href={profile.podcast} target="_blank" rel="noreferrer" className="transition-colors hover:text-magenta">Podcast</a>
          ...
          <a href="/shipfolio" className="font-semibold text-cyan transition-colors hover:text-text">ShipFolio ✦</a>
          <a href={`mailto:${profile.email}`} className="transition-colors hover:text-cyan">Email</a>
        </div>

        <p className="text-xs text-muted">
          {profile.name} · {profile.location} · Crafted with chaos, shipped with precision.
        </p>
