...
const BRANCHES = ["Leadership", "Delivery", "Entrepreneurship"] as const;

const BRANCH_COLOR: Record<(typeof BRANCHES)[number], string> = {
  Leadership: "var(--violet)",
  Delivery: "var(--cyan)",
  Entrepreneurship: "var(--amber)",
};
...
export default function SkillTree() {
  return (
    <Section id="skills" kicker="ability points allocated" title={<>Skill <span className="text-aurora">Tree</span></>}>
      <p className="-mt-4 mb-8 max-w-xl text-sm text-muted">
        Click a node to expand it — each one unlocks a real example. (Yes, opening them earns XP.)
      </p>
